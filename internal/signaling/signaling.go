...
type LiveSignaling struct {
	*ws.Ws
	listener    drobot_sdk_callback.OnSignalingListener
	loginUserID string
	db_interface.DataBase
	platformID int32
	isCanceled bool

	listenerForService drobot_sdk_callback.OnSignalingListener
...
func (s *LiveSignaling) SetListener(listener drobot_sdk_callback.OnSignalingListener) {
	log.Info("", utils.GetSelfFuncName(), "args ", listener)
	s.listener = listener
}

func (s *LiveSignaling) SetListenerForService(listener drobot_sdk_callback.OnSignalingListener) {
	log.Info("", utils.GetSelfFuncName(), "args ", listener)
	s.listenerForService = listener
}

func (s *LiveSignaling) getSelfParticipant(groupID string, callback drobot_sdk_callback.Base, operationID string) *api.ParticipantMetaData {
...
func (s *LiveSignaling) handleSignaling(req *api.SignalReq, callback drobot_sdk_callback.Base, operationID string) {
