...
NAME=./ws_wrapper/cmd/drobot_sdk_server
BIN_DIR=../../bin/
LAN_FILE=.go
...
ios:
	...
	rm -rf build/ drobot_im_sdk/t_friend_sdk.go drobot_im_sdk/t_group_sdk.go  drobot_im_sdk/ws_wrapper/
	go mod download golang.org/x/exp
	GOARCH=arm64 gomobile bind -v -trimpath -ldflags "-s -w" -o build/DrobotCore.xcframework -target=ios ./drobot_im_sdk/ ./drobot_im_sdk_callback/
...
android:
	...
	GOARCH=amd64 gomobile bind -v -trimpath -ldflags="-s -w" -o ./drobot_im_sdk.aar -target=android ./drobot_im_sdk/ ./drobot_sdk_callback/
