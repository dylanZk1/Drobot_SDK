...
func (wsRouter *WsFuncRouter) InitSDK(config string, operationID string) {
	...
	if userWorker.InitSDK(c, &initcb, operationID) {
		...
		//	wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), drobot_im_sdk.ErrCodeInitLogin, "init config failed", "", operationID})
	}
}

func (wsRouter *WsFuncRouter) UnInitSDK() {
	log.Info("", "UnInitSDK uid: ", wsRouter.uId)
	drobot_im_sdk.UserSDKRwLock.Lock()
	_, ok := drobot_im_sdk.UserRouterMap[wsRouter.uId]
	if ok {
		log.Info("", "delete UserRouterMap uid: ", wsRouter.uId)
		//*v = login.LoginMgr{}
	}
	delete(drobot_im_sdk.UserRouterMap, wsRouter.uId)
	log.Info("", "delete UnInitSDK uid: ", wsRouter.uId)
	drobot_im_sdk.UserSDKRwLock.Unlock()
}
...
func (wsRouter *WsFuncRouter) checkResourceLoadingAndKeysIn(mgr *login.LoginMgr, input, operationID, funcName string, m map[string]interface{}, keys ...string) bool {
	...
	if err := drobot_im_sdk.CheckResourceLoad(mgr); err != nil {
