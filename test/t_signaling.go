...
func DoTestInviteInGroup() {
	...
	drobot_im_sdk.SignalingInviteInGroup(t, t.OperationID, s)
}
...
func DoTestInvite(userID string) {
	...
	drobot_im_sdk.SignalingInvite(t, t.OperationID, s)
}
...
func DoTestAccept() {
	...
	drobot_im_sdk.SignalingAccept(t, t.OperationID, s)
}
...
func DoTestReject() {
	...
	s := utils.StructToJsonString(req)
	log.Info(t.OperationID, utils.GetSelfFuncName(), "input: ", s)
	drobot_im_sdk.SignalingReject(t, t.OperationID, s)
}
...
func DoTestCancel() {
	...
	s := utils.StructToJsonString(req)
	log.Info(t.OperationID, utils.GetSelfFuncName(), "input: ", s)
	drobot_im_sdk.SignalingCancel(t, t.OperationID, s)
}
...
func DoTestHungUp() {
	...
	s := utils.StructToJsonString(req)
	log.Info(t.OperationID, utils.GetSelfFuncName(), "input: ", s)
	drobot_im_sdk.SignalingHungUp(t, t.OperationID, s)
}

func DoTestSignalGetRoomByGroupID(groupID string) {
	t := testSingaling{baseCallback{OperationID: utils.OperationIDGenerator(), callName: utils.GetSelfFuncName()}}
	drobot_im_sdk.SignalingGetRoomByGroupID(t, t.OperationID, groupID)
}

func DoTestSignalGetTokenByRoomID(roomID string) {
	t := testSingaling{baseCallback{OperationID: utils.OperationIDGenerator(), callName: utils.GetSelfFuncName()}}
	drobot_im_sdk.SignalingGetTokenByRoomID(t, t.OperationID, roomID)
}
